...
#################################################
#DATA WRANGLING R - prepara√ß√£o do banco de dados# 
#################################################
...
plot(tx_desoc_ts,main='Taxa de Desocupa√ß√£o',
     ylab='Indice', xlab = 'Ano',
...
plot(rend_med_ts,main='Rendimento M√©dio',
     ylab='Indice', xlab = 'Ano',
...
############################################################################
#ao subtrair o IPCA^ da nossa s√©rie original, tamb√©m subtra√≠mos sua m√©dia. # 
#para normalizar a s√©rie, precisamos adicionar sua m√©dia.                  #
############################################################################

ipca.desn<-ipca.des+mean(fitted(ipca_reg))

################################################
#Gr√°fico IPCA original VS. IPCA dessazonalizado#
################################################
...
########################################################################
#TRANSFORMANDO A S…RIE EM ESTACIONARIA ATRAV…S DA REMO«√O DE TENDENCIAS#
########################################################################
...
plot(SAL_REAL_TS_DIFF,
     ...
     lty=2)
legend('topleft',
       c('Sal·rio MÌn Real', 'Sal·rio MÌn Real estacion·rio'),
       col=c('blue', 'red'), lty=1:2,
       bty='n')
...
plot(SAL_VIG_TS_DIFF,
     ...
     lty=2)
legend('topleft',
       c('Sal·rio MÌn Vigente', 'Sal·rio MÌn vigente estacion·rio'),
       col=c('blue', 'red'), lty=1:2,
       bty='n')
...
tx_desoc_ts<-tx_desoc_ts[,-1]
plot(tx_desoc_ts,main='Taxa de DesocupaÁ„o',
     ylab='Indice', xlab = 'Ano',
...
plot(tx_desoc_TS_DIFF,
     ...
     lty=2)
legend('topleft',
       c('Tx de desocupaÁ„o', 'Tx desocupaÁ„o estacion·rio'),
       col=c('blue', 'red'), lty=1:2,
       bty='n')
...
rend_med_ts<-rend_med_ts[,-1]
plot(rend_med_ts,main='Rendimento MÈdio',
     ylab='Indice', xlab = 'Ano',
     bty='l',col='blue',lty=1)+axis(1, at = c(2012:2022))
+axis(2, at = c(0:10))+grid(col='darkgrey',
                            lwd=2)
...
plot(rend_med_TS_DIFF,
     ...
     lty=2)
legend('topleft',
       c('Rendimento mÈdio', 'Rendimento mÈdio estacion·rio'),
       col=c('blue', 'red'), lty=1:2,
       bty='n')
...
##################################
#AN¡LISE DOS ÕNDICES POR GOVERNO#
#################################

#SEGUNDO GOVERNO LULA - 2007 AT… 2011#

#PIB
plot(PIB_TS_DIFF, xlab = "Ano", ylab = "Ìndice",  main = "PIB",las = 1, 
...
#sal·rio MÌn Vigente e Real 

par(mfrow=c(1,1))
plot(SAL_VIG_TS_DIFF, xlab = "Ano", ylab = "Ìndice",  main = "Sal·rio MÌn VIGENTE x REAL",las = 1, 
...
par(new=TRUE)
plot(SAL_REAL_TS_DIFF, xlab = "Ano", ylab = "Ìndice",  main = " ",las = 1, 
     xlim = c(2007,2011), ylim = c(-20, 120), xaxs = "i", yaxs = "i",
...
# Rendimento MÈdio - dados coletados a partir de 2012

# Taxa de desocupaÁ„o - Dados coletados ‡ partir de 2012

# IPCA VS IGPM

par(mfrow=c(1,1))
plot(IPCA_DESN_DIFF, xlab = "Ano", ylab = "Ìndice",  main = "IPCA X IGPM",las = 1, 
     xlim = c(2007,2011), ylim = c(-4, 7), xaxs = "i", yaxs = "i",
     cex.axis=1, bty='l',col='darkblue',lty=1, lwd = 2)
...
par(new=TRUE)
plot(igpm_TS_DIFF, xlab = "Ano", ylab = "Ìndice",  main = "",las = 1, 
     xlim = c(2007,2011), ylim = c(-4, 7), xaxs = "i", yaxs = "i",
     cex.axis=1, bty='l',col='darkred',lty=1, lwd =2)+grid(col='darkgrey',
                                                           lwd=1)+
  points(igpm_TS_DIFF, 
...
###############GOVERNO DILMA - 2011 - 2016 AT… AGOSTO ###################
...
plot(rend_med_TS_DIFF, xlab = "Ano", ylab = "Ìndice",  main = "Rendimento MÈdio",las = 1, 
...
plot(tx_desoc_TS_DIFF, xlab = "Ano", ylab = "Ìndice",  main = "Taxa DesocupaÁ„o",las = 1, 
...
plot(IPCA_DESN_DIFF, xlab = "Ano", ylab = "Ìndice",  main = "IPCA",las = 1, 
     xlim = c(2011,2016), ylim = c(-4, 7), xaxs = "i", yaxs = "i",
...
plot(igpm_TS_DIFF, xlab = "Ano", ylab = "Ìndice",  main = "IGPM",las = 1, 
     xlim = c(2011,2016), ylim = c(-4, 7), xaxs = "i", yaxs = "i",
     cex.axis=1, bty='l',col='darkblue',lty=1)+grid(col='darkgrey',
                                                    lwd=2)+
  points(igpm_TS_DIFF, 
...
##########GOVERNO TEMER - AGOSTO 2016 AT… 2018 ###############
